...
// https://sbcode.net/typescript/template/#source-code
abstract class AbstractClass {
  // A template class containing a template method and
  // primitive methods
  stepOne(): void {
    // Hooks are normally empty in the abstract class. The
    // implementing class can optionally override providing
    // a custom implementation
  }
  abstract stepTwo(): void;
  // An abstract method that must be overridden in the
  // implementing class. Note the addition of
  // the `abstract` keyword
  stepThree(): void {
    // Hooks can also contain default behavior and can be
  ...
  templateMethod() {
    ...
    // The subclass(implementing class) doesn't need to
    // override this method since it has would have already
    // optionally overridden the following methods with its
    // own implementations
    this.stepOne();
    this.stepTwo();
    this.stepThree();
...
class ConcreteClassB extends AbstractClass {
  // A concrete class that only overrides steps one, two
  // and three
  stepOne() {
